...
}if(N.metaKey||N.altKey||N.ctrlKey){return true}var I,O,L=f.getElementsByTagName("tr")[0].getElementsByTagName("td").length,H;
if(e){var M=e.fnSettings();H=M.aiDisplay.length;var K=g(m);if(K===null){return}c=K[0];
a=K[1]}else{H=f.getElementsByTagName("tr").length}var J=(N.keyCode==9&&N.shiftKey)?-1:N.keyCode;
switch(J){case 13:N.preventDefault();N.stopPropagation();A("action",c,a);return true;
case 27:if(!A("esc",c,a)){u()}break;case -1:case 37:if(c>0){I=c-1;O=a}else{if(a>0){I=L-1;
O=a-1}else{if(J==-1&&n){C=true;s.focus();setTimeout(function(){C=false},0);w=false;
u();return true}else{return false}}}break;case 38:if(a>0){I=c;O=a-1}else{return false
