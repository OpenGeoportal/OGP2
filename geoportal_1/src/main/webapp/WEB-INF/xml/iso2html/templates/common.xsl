...
<xsl:stylesheet version="1.0"
		...
		<xsl:choose>
			...
			<xsl:when test="$level = 2">
				...
				<h3>
					<xsl:attribute name="title">
      					<xsl:value-of select="name()"/>
    				</xsl:attribute><xsl:value-of select="$heading"/></h3>
			</xsl:when>
			<xsl:when test="$level = 4">
				<h4>				   
					<xsl:attribute name="title">
      					<xsl:value-of select="name()"/>
    				</xsl:attribute><xsl:value-of select="$heading"/></h4>
			</xsl:when>
			...
			<xsl:otherwise>
				...
				<xsl:attribute name="title">
      					<xsl:value-of select="name()"/>
    				</xsl:attribute><xsl:value-of select="$heading"/></h6>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template name="ciElement">
		...
		<xsl:call-template name="elementHeader">
			...
			<xsl:with-param name="heading"><xsl:value-of select="concat($heading, ':')"/></xsl:with-param>
		</xsl:call-template>
			<div>
				<xsl:choose>
