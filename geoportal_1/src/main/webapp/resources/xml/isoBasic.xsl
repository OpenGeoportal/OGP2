...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0" xmlns:gco="http://www.isotc211.org/2005/gco" xmlns:gmd="http://www.isotc211.org/2005/gmd" xmlns:srv="http://www.isotc211.org/2005/srv" xmlns:gml="http://www.opengis.net/gml/3.2" xmlns:gts="http://www.isotc211.org/2005/gts" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">  
  ...
  <a name="Top"/>
    ...
    <h1><xsl:value-of select="gco:CharacterString"/></h1>
  	</xsl:for-each>
    <xsl:for-each select="//gmd:MD_DataIdentification/gmd:abstract">
    	<h4>Abstract:</h4>
    	<p><xsl:value-of select="gco:CharacterString"/></p>
    </xsl:for-each>  
  ...
  <!--Create keywords indices (keys) so that we can do a unique sort below: -->
   ...
   <!-- CONTENT_INFORMATION: *************************************************-->
    ...
    <xsl:for-each select="//gmd:extent/gmd:EX_Extent">
      <b><i>Spatial Temporal Extent:</i></b>
      <blockquote>
      ...
      <xsl:for-each select="gmd:verticalElement">
    	...
    	<xsl:for-each select="gmd:CI_ResponsibleParty/gmd:role" > 
    		<xsl:value-of select="gmd:CI_RoleCode/@codeListValue" /><br/>
    	</xsl:for-each>
    	...
    	<xsl:for-each select="gmd:CI_ResponsibleParty/gmd:contactInfo/gmd:CI_Contact">
    	<br/>
    	...
    	<xsl:for-each select="gmd:address/gmd:CI_Address/gmd:electronicMailAddress" > 
    		<i><xsl:value-of select="gco:CharacterString"/></i><br/>
    	</xsl:for-each>
 </xsl:for-each>
 </p>   		
    </xsl:for-each>  
