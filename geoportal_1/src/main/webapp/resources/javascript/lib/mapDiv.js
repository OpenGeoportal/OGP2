 ...
 * defined under the object MapController. MapController inherits from the
...
OpenGeoportal.MapController = function() {
		...
		var options = {
			...
			controls : [ new OpenLayers.Control.ModPanZoomBar(),
					new OpenLayers.Control.ScaleLine({
						geodesic : true
					}), displayCoords, nav, this.createDefaultOLPanel(nav) ]
		};
	...
	this.initMap = function() {
		...
		defaultBasemapModel.get("initialRenderCallback").apply(
				defaultBasemapModel, [ this ]);
		...
		jQuery(".olMap").find("[id*=event]").addClass("shadowDown").addClass(
				"shadowRight");
	...
	this.mapControlUIHandler = function() {
		...
		jQuery(document).on(
				...
				function() {
					jQuery(this).attr("src",
							that.utility.getImage("slider_plus_hover.png"));
		...
		jQuery(document).on(
				...
				function() {
					jQuery(this).attr("src",
							that.utility.getImage("zoom-plus-mini.png"));
				});
		...
		jQuery(document).on(
				...
				function() {
					jQuery(this).attr("src",
							that.utility.getImage("slider_minus_hover.png"));
					// jQuery(this).css("cursor", "pointer");
				});
		...
		jQuery(document).on(
				...
				function() {
					jQuery(this).attr("src",
							that.utility.getImage("zoom-minus-mini.png"));
				});
	...
	this.saveImage = function(imageFormat, resolution) {
		...
		// this doesn't really work... should get appropriate width and height
		// based on bbox
	...
	/**
	 * sets the background map to the value in the background map dropdown menu.
	 * called by change for the basemap radio button set
	 */
	/*
	 * this.basemapHandler = function(){ var that = this;
	 * jQuery("[name=basemapRadio]").on("change", function(){ var value =
	 * jQuery('input:radio[name=basemapRadio]:checked').val();
	 * that.basemaps.findWhere({name: value}).set({selected: true}); }); };
	 */

	/*
	 * this.getBackgroundType = function() { var layers = this.layers; for (var
	 * i in layers){ if ((layers[i].CLASS_NAME ==
	 * "OpenLayers.Layer.Google")&&(layers[i].visibility == true)
	 * &&layers[i].opacity == 1){ return layers[i].type; } else { return "osm"; } } };
	 */
			...
			initialRenderCallback : function(mapController) {
				...
				google.maps.event.addListener(bgMap.mapObject, "tilesloaded",
						function() {
							...
							// really should only fire the first time (or should
							...
							jQuery("[id$=GMapContainer]").find(
									'[title*="Click to see this area"]')
									.parent().addClass("googleLogo");
		...
		var bingRoad = {
			...
			showOperations : function(model) {
				...
				if (that.getLayersBy("basemapType", model.get("type")).length === 0) {
					...
					// var layer = that.getLayersBy("basemapType",
					// model.get("type"))[0];
					// layer.setVisibility(true);
	...
	this.previewOnDispatcher = function(location) {
		...
		// there's no preview method identified, so just return a function that
		// shows a static bounding box
		return defaultObj;
	};
	...
	this.previewLayerOn = function(layerId) {
		...
		}); // perhaps this should happen on model add
