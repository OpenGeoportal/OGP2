...
OpenGeoportal.Views.PreviewTools = Backbone.View.extend({
	...
	updateSlider: function(controlClass, newValue){
		...
		//update slider value
		this.$el.find("." + controlClass).find(".previewToolsSlider").slider("option", "value", newValue);
	},
	initRender: function(){
		...
		if (this.model.has("graphicWidth")){
			...
			markup += template.sliderControl({controlClass: "sizeControlCell", label: label, value: this.model.get("graphicWidth"), units: "px", tooltip: tooltip});
		}		
		if (this.model.has("color")){
			//render a color control
			markup += template.colorControl({color: this.model.get("color")});
		}
		...
		if (this.model.has("getFeature")){
			...
			markup += template.getFeatureControl({toolClass: toolClass});
		}
		
		this.$el.html(template.previewTools({toolsMarkup: markup}));
	...
	createOpacitySlider: function(){
		...
		//class = opacitySlider
		var that = this;
		this.$el.find(".opacityControlCell").find(".previewToolsSlider").slider({
			min: 0,
			max: 100,
			step: 5,
			...
			slide: function(event, ui) {
				...
				var text$ = jQuery(this).parent().parent().find(".sliderValue");
				text$.text(ui.value);
			},
	...
	toggleFeatureInfo: function(event){
		...
		if (!getFeature){
			//update layer state
			this.model.set({getFeature: true});
		} else {
...
OpenGeoportal.Views.ColorPicker = Backbone.View.extend({
	...
	selectColorCell: function(event){
		...
		this.model.set({color: selectedColor});	//here's where things happen
