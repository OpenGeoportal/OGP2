...
OpenGeoportal.Models.CartLayer = OpenGeoportal.Models.DataTypeAware.extend({
	...
	requiresEmailAddress : function() {
		// there should be a more generalized way to do this, rather than
		// specifying "Harvard"
		if ((this.get("Institution").toLowerCase() === "harvard")
				&& (this.get("baseType") === "raster")) {
			this.set({
...
OpenGeoportal.CartCollection = Backbone.Collection
		.extend({
			...
			isActionAvailable : function(model, action) {
				// returns true for passed models that have passed 'action'
				// available
				var filter = function() {
					return true;
			...
			downloadFilter : function(model) {
				// no external restricted layers, no local restricted layers if
				// not logged in.
				...
				// what's a better way of determining this?
				// availability: online ? is this accurate for our records?
			...
			intersectsBounds : function(model, bounds) {
				// return an array of models in the collection that intersect
				// the passed bounds
				...
				// console.log(model);
				// console.log(bounds);// would prefer for bounds to be an
				// object,
