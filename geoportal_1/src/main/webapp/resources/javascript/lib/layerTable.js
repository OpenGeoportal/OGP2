...
OpenGeoportal.LayerTable = function LayerTable(){
	...
	this.createTableHeadings = function(){
		...
		tableConfigCollection.listenTo(tableConfigCollection, "change:width", function(){that.setColumnWidth.apply(that, arguments);});
	...
	this.tableDrawCallbacks = {
			callbacks: {
				...
				adjustColumns: function(){this.adjustColumnSizes();}
			}
	};
	...
	this.adjustColumnSizes = function(){
		...
		var remainingWidth = jQuery("#" + this.getWrapperDiv()).width() - this.getFixedColumnsWidth();
		//console.log("***********************************************************");
		...
		//console.log("remaining width minus minwidths: " + (remainingWidth - totalMinWidth));
		...
		for (var j in resizables){
			...
			if (newWidth >= currSize.minWidth){
				resizables[j].set({width: newWidth});
				minWidthCarryOver = 0;
			} else {
				resizables[j].set({width: currSize.minWidth});
				console.log("minWidthCarryOver: " + minWidthCarryOver);
				console.log("diff bw min and new: " + parseInt(currSize.minWidth - currSize.newWidth));
				...
				//console.log("set width as min width, remainder: " + minWidthCarryOver);
			}
		}
	...
	this.highlightRow = function(){
		...
		jQuery(document).on("mouseover", '#' + tableName + ' > tbody', function(event) {
			...
			var bbox = {};
			var currModel = that.backingData.findWhere({LayerId: aData.LayerId});
			...
			if (typeof currModel !== "undefined"){
				...
				console.log("Could not find model with LayerId " + aData.LayerId + " in the collection.");
			}
		});
	};
	...
	this.expandViewHandler = function(){
		...
		jQuery("#" + that.getTableId()).on("view.openRow", function(event, data){
			...
			var previewModel = that.backingData.findWhere({LayerId: data.LayerId});
			if (typeof previewModel === "undefined"){
				console.log("the model with LayerId: " + data.LayerId + " could not be found in the collection");
			}
			
			var layerModel = that.previewed.getLayerModel(previewModel);//at this point, we need to have a model for the layer in "previewed", or we can't render the tools properly
	...
	this.addToPreviewed = function(layerId){
		...
		if (typeof model == "undefined"){
			...
			var layerAttr;
			try {
				var layerModel = this.backingData.findWhere({LayerId: layerId});
				if (typeof layerModel === "undefined"){
					throw new Error("Could not find model with LayerId " + layerId + " in the collection.");
				}
