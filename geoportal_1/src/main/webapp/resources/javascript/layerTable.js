 ...
 */
OpenGeoportal.LayerTable = function(stateObj){
	...
	this.tableHeadingsObj = new OpenGeoportal.LayerTable.TableHeadings(this);
	this.tableLayerState = new OpenGeoportal.LayerTable.TableLayerState(this);
	this.template = new OpenGeoportal.Template();	//this should be app-wide
	...
	this.createDataTable = function(table$, tableData, columnDefs){
		var that = this;
		var tableObj = table$.dataTable( {
			"iDisplayLength": that.layersToDisplay.buffer, 	//*******Search Results only
			"fnDrawCallback": that.tableDrawCallbacks,
			"bAutoWidth": false,
			"sDom": 'rt',
			"aaData": tableData,
			"bSort": false,
	...
	this.createTable = function(div){
		var table$ = this.createTableHtml(div);	   
		//initialize table
		var tableData = this.processData(this.getInitData());
		var columnDefinitions = this.getColumns();
		//table created
		var tableObj = this.createDataTable(table$, tableData, columnDefinitions);
		return tableObj;
	};

	this.initTable = function(div){
		var tableObj = this.createTable(div);
		//tableObj.fnClearTable();
		this.initControlHandlers();
	};

	this.tableDrawCallbacks = function(){
		//console.log(this);
		//console.log(that);
		that.wrapCells();
		that.createTooltips();
	...
	this.getColumnData = function(rowData, colName){
		var columnIndex = this.tableHeadingsObj.getColumnIndex(colName);
	...
	this.findTableControl = function(layerId, columnClass, controlClass){
		...
		jQuery(columnClass).each(function(){
			...
			var currentLayerId = aData[that.tableHeadingsObj.getColumnIndex("LayerId")];
	...
	this.getPreviewToolHTML = function(rowNode){
		...
		var displayName = OpenGeoportal.Utility.escapeQuotes(rowData[this.tableHeadingsObj.getColumnIndex("LayerDisplayName")]);
	...
	//the problem with this is that it combines updating the model and updating the view...
	//how can we make these things distinct?
	...
	this.previewedLayers = OpenGeoportal.PreviewedLayers;
	...
	//what do I need to know about a layer to preview it?
	...
	this.previewLayer = function(thisObj){
		...
		if (layerState.getState(layerID,"preview") == "off"){
			...
			try {
				...
				new OpenGeoportal.ErrorObject(err,'Preview parameters are invalid.  Unable to Preview layer "' + this.getColumnData(aData,"LayerDisplayName") +'"');
			}
			...
			if (OpenGeoportal.ogp.map.getLayersByName(layerID)[0]){
				OpenGeoportal.ogp.map.showLayer(layerID);
				OpenGeoportal.ogp.map.getLayersByName(layerID)[0].setOpacity(opacitySetting * .01);
				...
				if (availability.toLowerCase() == "online"){
					if (typeof location.wms != "undefined"){
						...
						OpenGeoportal.ogp.map.addWMSLayer(mapObj);
						...
						OpenGeoportal.ogp.map.addArcGISRestLayer({"institution": institution, "layerName": layerName, "title": layerID, 
					...
					//console.log(mapObj);
					OpenGeoportal.ogp.map.addMapBBox(mapObj);
					layerState.setState(layerID, {"preview": "on", "dataType": dataType, "wmsName": layerName});
				}
			}
			...
			try {
				...
				OpenGeoportal.ogp.map.hideLayer(layerID);

				layerState.setState(layerID, {"preview": "off"});

				this.previewedLayers.removeLayer(layerID, index);

			} catch (err) {new OpenGeoportal.ErrorObject(err, "Error turning off preview.");};
	...
	this.getPreviewControl = function (rowObj){
		...
		if (layerAccess == "Public"){
			...
			var homeInstitution = OpenGeoportal.InstitutionInfo.getHomeInstitution();
	...
	this.zoomToLayerExtentHandler = function(){
		jQuery("#" + that.getTableID() + " tbody").on("click.zoomToLayerExtent", "div.zoomToLayerControl", function(event){
			...
			OpenGeoportal.ogp.map.zoomToLayerExtent(bbox);
		});
	};

	this.saveHandler = function(){
		jQuery("#" + that.getTableID() + " tbody").on("click.save", "div.saveControl", function(event){
			that.saveLayer(this);
		});
	};
	...
	this.getColumns= function(){
		...
		for (var heading in this.tableHeadingsObj.getTableHeadings()){
...
//	*******Search Results only
	...
	this.searchRequestJsonp = function(startIndex){
		...
		var searchType = OpenGeoportal.Utility.whichSearch().type;
...
//******Table Specific
OpenGeoportal.LayerTable.TableLayerState = function(thisObj){
...
//******Table Specific
OpenGeoportal.LayerTable.TableHeadings = function(thisObj){
	var defaultHeadings = {
			...
			{"sName": "checkBox", "sTitle": "<input type=\"checkbox\" id=\"downloadHeaderCheck\" checked />", "bVisible": false, "aTargets": [ 1 ], "sClass": "colChkBoxes", "sWidth": "21px", "bSortable": false,
				...
				{"sName": "expandControls", "sTitle": "", "bVisible": true, "aTargets": [ 2 ], "sClass": "colExpand", "sWidth": "10px", "bSortable": false,
						...
						{"sName": "DataType", "sTitle": "Type", "bVisible": true, "aTargets": [ 5 ], "sClass": "colType", "sWidth": "24px", "bSortable": false, "bUseRendered": false, 
							...
							{"sName": "ContentDate", "sTitle": "Date", "bVisible": false, "aTargets": [ 10 ], "sClass": "colDate", "sWidth": "25px", "bSortable": false, "bUseRendered": true,
								...
								{"sName": "Institution", "sTitle": "Rep", "bVisible": true, "aTargets": [ 11 ], "sClass": "colSource", "sWidth": "19px", "bSortable": false, "bUseRendered": false, 
									...
									{"sName": "Metadata", "sTitle": "Meta", "bVisible": true, "aTargets": [ 12 ], "sClass": "colMetadata", "sWidth": "23px", "bSortable": false, "bUseRendered": false, 
